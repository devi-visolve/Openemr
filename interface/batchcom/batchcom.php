...
if ($_POST['form_action']=='Process') {
    ...
    if (!$form_err) {
        ...
        //   ";
           $sql="select patient_data.*, MAX( cal_events.pc_endDate ) AS last_ap, MAX( forms.date) AS last_visit, (DATEDIFF(CURDATE(),patient_data.DOB)/365.25) AS pat_age   from patient_data join openemr_postcalendar_events AS cal_events on patient_data.pid=cal_events.pc_pid join forms on patient_data.pid=forms.pid
           ";
 ...
 <td>&nbsp;</td>
  ...
  <td>&nbsp;</td><td>&nbsp;</td>
   ...
   <tr><td colspan='4'>
    ...
    <INPUT TYPE="submit" name="form_action" value=<?php xl('Process','e','\'','\''); ?>> </td><td><?php xl('Takes time','e')?></td> <td>&nbsp;</td><td>&nbsp;</td></tr>
</table>
</div>
</div>
</FORM>
