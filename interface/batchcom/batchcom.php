...
if ($_POST['form_action']=='Process') {
    ...
    if (!$form_err) {
          ...
          // $sql="select patient_data.*, cal_events.pc_eventDate as next_appt, MAX( cal_events.pc_endDate ) AS last_ap, MAX( forms.date) AS last_visit, (DATEDIFF(CURDATE(),patient_data.DOB)/365.25) AS pat_age   from patient_data left join openemr_postcalendar_events AS cal_events on patient_data.pid=cal_events.pc_pid join forms on patient_data.pid=forms.pid";
